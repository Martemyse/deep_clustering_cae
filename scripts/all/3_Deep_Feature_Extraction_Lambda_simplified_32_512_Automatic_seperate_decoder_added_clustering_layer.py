...
layers_depths=[8, 16, 56, 6]
layer_depth_latent_cnn = 6
...
def autoencoderConv2D_1(input_shape= (32,512)):
    ...
    conv_1_1 = Conv2D(layers_depths[0], conv2Dfilters[0], activation = 'relu', padding = "SAME", strides=(1, stride_norm_2dconv))(inputs_1)
    ...
    conv_latent_1 = Conv2D(layer_depth_latent_cnn, ((pool_1_2.get_shape().as_list())[1:][0],(pool_1_2.get_shape().as_list())[1:][1]), activation = 'relu', padding = "SAME", strides=(1, stride_latent_2dconv))(pool_1_2)
    ...
    dense_latent_4 = Dense(layers_depths[2], activation = 'relu')(clustering_layer)
    ...
    conv_latent_2 = Conv2DTranspose(layer_depth_latent_cnn, ((pool_1_2.get_shape().as_list())[1:][0],(pool_1_2.get_shape().as_list())[1:][1]), activation = 'relu', padding = "SAME", strides=(1, stride_latent_2dconv))(upsampling_latent_1)
    ...
    dec_upsampling_1_1 = UpSampling2D((max_pool_filter_h,max_pool_filter_w))(conv_latent_2)
    ...
    output_1 = Conv2DTranspose(1, conv2Dfilters[0], activation = 'relu', padding = "SAME", strides=(1, stride_norm_2dconv))(dec_upsampling_1_2)
...
zoom_factors = (1,32 / cwtms_bio_cold.shape[1], 128 / cwtms_bio_cold.shape[2])
...
save_dir = save_dir_root + '\Clustering_CAE_isndcm_seperate_decoder_lambda_32x512_{filename}_v2'.format(filename=filename) +  ver_str + '.h5'
...
save_dir = save_dir_root + '\\Clustering_CAE_isndcm_seperate_decoder_lambda_32x512_45-02s-13_v2_16_32_12_128_6_30_128' + '.h5'
...
intermediate_layer_model = Model(inputs=model.input, outputs=model.get_layer(index=28).output)
    ...
    if ite % update_interval == 0:
        ...
        for cluster_num in cluster_nums:
            ...
            if cluster_num == 4:
                ...
                fig.add_axes(ax)
                plt.cla()
                for name, label in [('Category1', 0), ('Category2', 1), ('Category3', 2), ('Category4', 3)]:
                    a = 0
                # labels_clusters3 = np.choose(labels_kmeans, [0, 2, 1]).astype(float)
                unsupervised_labels = np.choose(y_pred, [0, 1, 2,3,4,5,6,7]).astype(float) #pac channel 1
                ...
                ax.scatter(intermediate_output[:, 0], intermediate_output[:, 1], intermediate_output[:, 2], c=unsupervised_labels,cmap = mcolors.ListedColormap(viridis_cols), label = unsupervised_labels, edgecolors='none', s=class_marker_sizes)
                ax.w_xaxis.set_ticklabels([])
                ax.w_yaxis.set_ticklabels([])
                ax.w_zaxis.set_ticklabels([])
                plt.title('4 clusters K-means')
                plt.show() 
