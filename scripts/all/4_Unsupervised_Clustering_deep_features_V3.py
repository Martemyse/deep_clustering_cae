...
range_n_clusters = 5
identifier = f'chatGPT_32x64_may_rmk_{range_n_clusters}_clusters_v1'
...
# features_merged = pd.read_pickle(r"C:\Users\Martin-PC\Biocomposite_analysis_isndcm\merged_features_encoder_clustering_chatGPT_32x64_w7_{filename}_32x64_36_36_16_512_6_10_32.pkl".format(filename=filename))
# features_merged = pd.read_pickle(r"C:\Users\Martin-PC\Biocomposite_analysis_isndcm\merged_features_encoder_clustering_chatGPT_32x64_w7_{filename}_32x64_energy_all_36_36_16_512_6_10_32.pkl".format(filename=filename))
# features_merged = pd.read_pickle(r"C:\Users\Martin-PC\Biocomposite_analysis_isndcm\merged_features_encoder_clustering_chatGPT_32x64_5_clusters_v5_{filename}_32x64_energy_all_36_36_16_512_6_10_32.pkl".format(filename=filename))
# features_merged = pd.read_pickle(r"C:\Users\Martin-PC\Biocomposite_analysis_isndcm\merged_features_encoder_clustering_chatGPT_32x64_july_last_6_layers_train_3_clusters_v1_45-02s-13_32x64_energy_all_40_40_14_512_3_10_32.pkl".format(filename=filename))

# features_merged = pd.read_pickle(r"C:\Users\Martin-PC\Biocomposite_analysis_isndcm\Feature Analysis Clustering Layer emb 150 300 500 2000\6 clusters\45-02s-11\hits_channel1_df_labels_encoder_clustering_chatGPT_32x64_6_clusters_v1_45-02s-11_32_16_8_256_6_100_256.pkl".format(filename=filename))
features_merged = pd.read_pickle(r"C:\Users\Martin-PC\Biocomposite_analysis_isndcm\Feature Analysis Clustering Layer emb 150 300 500 2000\6 clusters\45-02s-13\hits_channel1_df_labels_encoder_clustering_chatGPT_32x64_6_clusters_v3_45-02s-13_32_16_8_256_6_100_256.pkl".format(filename=filename))
...
features_merged['Relative time [%]'] = (features_merged['Time of Event']/max(features_merged['Time of Event']))*100
...
#
indices1=list(np.where((features_merged['Relative time [%]']>=50))[0])
features_merged = features_merged.iloc[indices1].reset_index(drop=True)
#%
...
# deep_features_to_keep = [5,8,17,23,25,26,27] # amplitude,energy,risetime,FMXA,deeps
# deep_features_to_keep = [4,6,9,18,24,25,26] # amplitude,energy,risetime,deeps
# deep_features_to_keep = [4,6,9,18,24]  # just deep clusters 4
# deep_features_to_keep = [25,27,28] # just deep clusters 5 v5 13ka
# deep_features_to_keep = [25,27,28] # just deep clusters 5 v5 11ka
# deep_features_to_keep = [24,25,27,28] # just deep clusters 6 v3 11ka
# deep_features_to_keep = [24,25,27] # just deep clusters 6 v3 11ka
# deep_features_to_keep = [25,27,28] # just deep clusters 6 v3 13ka
# deep_features_to_keep = [24,25,26] # just deep clusters 6 v3 11ka
# deep_features_to_keep = [24,25,27,28] # just deep clusters 6 v2 11ka
# deep_features_to_keep = [25,27,28] # just deep clusters 6 v2 11ka

# filename = '45-02s-11'
# deep_features_to_keep = [24,25,27,29] # just deep clusters 6 v2 11ka
# deep_features_to_keep = [4,6,9,24,25,29] # just deep clusters 6 v2 11ka

# filename = '45-02s-13'
# deep_features_to_keep = [25,27,28] # just deep clusters 6 v2 11ka
deep_features_to_keep = [4,6,9,25,27,28] # just deep clusters 6 v2 11ka
...
# # db = DBSCAN(eps=0.5, min_samples=100).fit(X) # PCA

# db = DBSCAN(eps=20, min_samples=100).fit(X) # Robust Scaler, klasične značilke

# # db = DBSCAN(eps=2, min_samples=100).fit(X) # Robust Scaler, samo globoke
# # db = DBSCAN(eps=0.28, min_samples=100).fit(X) # Robust Scaler, standardScaler, PCA
# # db = DBSCAN(eps=1.58, min_samples=100).fit(X) # Robust Scaler, standardScaler, PCA
...
for n_clusters in range_n_clusters:
    ...
    plt.style.use('dark_background')
    #deep features
    dim_x = 0 #energija deep
    dim_y = 1 # globoka 1 deep
    dim_z = 2 # FCOG deep
    ...
    # viridis_cols = ['#440154','#365c8d','#1fa187','#dde318','tomato','magenta']
    # viridis_cols = ['#440154','tomato','#dde318']
...
# feature = 'Log Energy in Signal'

#%%
hits_channel1_df = hits_channel1_df.rename(columns={'time':'Time of Event','energy':'Energy in Signal(au)'})
hits_channel1_df['Log Energy in Signal'] = np.log(hits_channel1_df['Energy in Signal(au)'])
hits_channel1_df['Log Energy in Signal'] = np.abs(hits_channel1_df['Log Energy in Signal'])
...
# energy_column = 'Energy in Signal(au)'
...
for z_axis in ['Log Energy in Signal','amplitude','rise_time']:
    ...
    with open(r"C:\Users\Martin-PC\Biocomposite_analysis_isndcm\3dscatter_{identifier}_{filename}_{z_axis}.html".format(filename=filename,z_axis=z_axis,identifier=identifier), "w", encoding='utf-8') as file:
...
#%%
# featurex = 'Energy in Signal(au)'
featurex = 'Log Energy in Signal'
...
ax.legend()
ax2=ax.twinx()
ax2.plot(hits_channel1_df['Time of Event'], hits_channel1_df[featurex], color="gray", marker = '.', linestyle = 'None', markersize = 0.3)
...
save_dir = save_dir_root + '\hits_channel1_df_labels_{identifier}_{filename}'.format(filename=filename,identifier=identifier) +  ver_str + '.xlsx'
hits_channel1_df_labels.to_excel(save_dir, index = False) 
pd.to_pickle(hits_channel1_df_labels, save_dir.replace('xlsx','pkl'))
#%%
# filename = '45-02s-11'
cwtms = pd.read_pickle(r"C:\Users\Martin-PC\Biocomposite_analysis_isndcm\{filename}_32_512.pkl".format(filename=filename))
...
for indc,cluster_id in enumerate(['Cluster {}'.format(i+1) for i in range(n_clusters)]):
    # breakpoint()
    hits_channel1_df_labels_cluster=hits_channel1_df_labels[hits_channel1_df_labels['label_unsupervised'].isin([cluster_id])]
    cwtms_clusters[indc] = cwtms[hits_channel1_df_labels_cluster['index'].values,:,:]
