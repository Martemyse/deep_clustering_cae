...
x1_training_input_batch = np.concatenate((cwtms_tok,cwtms_t80),axis=0)
# x1_training_input_batch = cwtms
...
#                                     # steps_per_epoch=len(x1_training_input_batch) // 32
#                                     )
...
#%%    
autoencoder.fit(datagen.flow(x1_training_input_batch, x1_training_input_batch, batch_size=12), 
                            epochs=train_epochs, # one forward/backward pass of training data
                            shuffle=True,
                            batch_size = batch_size,
                            steps_per_epoch=len(x1_training_input_batch) // 24
...
save_dir = save_dir_root + '\yUpsilon_retrained_20epochs_on_Tok' +  ver_str + '.h5'
...
# predicted_x2_filtered_input = cwtms_ch1_pure_input[:number_of_predicted,:,:]
# for img in range(number_of_predicted):
#     predicted = autoencoder.predict([predicted_x2_filtered_input])
#     image_decropped = predicted.reshape((number_of_predicted,BatchesSliceHeight,BatchesLength))
#     fs = 5000000
...
#     previous_time_data = np.arange(0,BatchesLength)*dt
#     freq = np.linspace(1, fs/5, BatchesSliceHeight)
#     plt.pcolormesh(previous_time_data, freq, cwtms_ch1_pure[img,:,:], cmap='viridis', shading='gouraud')
...
# autoencoder.compile(optimizer='adam', loss='binary_crossentropy')
autoencoder.load_weights(save_dir+'\yUpsilon_ch1_16_32_14_256_6_100_256.h5')
train_epochs = 100
#%% adding classical features CFE @@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@
# breakpoint()
x_pure_pred_enc = encoder.predict(cwtms_ch2_tok_input)
#%
# er
...
# classical_features_filtered_hits_ch1_cfe = pd.read_pickle(r"C:\Users\Martin-PC\Magistrska Matlab\cwtms_ch1_ch2_classical_features.pkl")
...
save_dir = save_dir_root + '\merged_features_embedded_classical_ch1' +  ver_str + '.pkl'
pd.to_pickle(merged_features_embedded_classical, save_dir)
save_dir = save_dir_root + '\merged_features_embedded_classical_ch1' +  ver_str + '.xlsx'
