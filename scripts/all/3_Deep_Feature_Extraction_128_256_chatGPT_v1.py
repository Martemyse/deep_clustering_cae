...
def autoencoderConv2D_1(input_shape= (128,64),
                        layers_depths=[128, 128, 1024, 128, 512, 256],
                        layer_depth_latent_cnn=20
                        ):
    # layers_depths=[36, 18, 256, 6]
    # layer_depth_latent_cnn = 10
    conv2Dfilters = [(5,5), (5,5)]
    BatchesSliceHeight = 128
    BatchesLength = 64
    stride_pool_h = 2
    stride_pool_w = 2
    max_pool_filter_h = 2
    ...
    conv_latent_2 = Conv2D(layers_depths[1], conv2Dfilters[0], activation = 'relu', padding = "SAME", strides=(1, stride_norm_2dconv))(pool_latent_1)
    pool_latent_2 = MaxPooling2D(pool_size = (max_pool_filter_h,max_pool_filter_w), strides = (stride_pool_h,stride_pool_w))(conv_latent_2)
    conv_latent_3 = Conv2D(layers_depths[1], conv2Dfilters[1], activation = 'relu', padding = "SAME", strides=(1, stride_norm_2dconv))(pool_latent_2)
    pool_latent_3 = MaxPooling2D(pool_size = (max_pool_filter_h,max_pool_filter_w), strides = (stride_pool_h,stride_pool_w))(conv_latent_3)
    flat_latent_1  = Flatten()(pool_latent_3)
    ...
    # clustering_layer = ClusteringLayer(n_clusters=5)(dense_latent_bottleneck)
    # dense_latent_a = Dense(layers_depths[5], activation = 'relu')(clustering_layer)
    # dropout_latent_a = Dropout(dropout_ratio)(dense_latent_a)
    # dense_latent_b = Dense(layers_depths[4], activation = 'relu')(dropout_latent_a)
    # dropout_latent_b = Dropout(dropout_ratio)(dense_latent_b)
    dense_latent_c = Dense(layers_depths[2], activation = 'relu')(encoder_output_linear_regression)
    ...
    return AE, encoder, encoder_linear_regression
...
encoder_linear_regression.summary()

# opt = keras.optimizers.Adam(learning_rate=0.0001)
opt = keras.optimizers.Adam(learning_rate=0.0003)
save_dir = r'C:\Users\Martin-PC\\Biocomposite_analysis_isndcm\Clustering_CAE'
encoder_linear_regression.compile(optimizer=opt, loss='mse') #mse različn način kako se penalizira razlikovanje vhodnega in izhodnega spektograma
...
#%% retraining encoder
save_dir = save_dir_root + '\\Clustering_CAE_encoder_clustering_chatGPT_32x64_6_clusters_all_files_v1_36_36_16_512_6_10_32' + '.h5'
...
identifier = 'encoder_clustering_chatGPT'
loss = 0
index = 0
# maxiter = 500
# maxiter = 8000
...
for ite in range(int(maxiter)):
    ...
    x_input_batch = x_input[idx]
    x_input_batch_3d = x_input[idx][:,:,:,0]
...
ver_str = "_" + str(layers_depths[0]) + "_" + str(layers_depths[1]) + "_" + str(layer_depth_latent_cnn) + "_" + str(layers_depths[2]) + "_" + str(layers_depths[3]) + "_" + str(train_epochs) + "_" + str(batch_size)
save_dir_root = r'C:\Users\Martin-PC\Biocomposite_analysis_isndcm\Clustering_CAE'
save_dir = save_dir_root + '\Clustering_CAE_encoder_clustering_chatGPT_32x64_6_clusters_{filename}_v2'.format(filename=filename) +  ver_str + '.h5'
encoder.save_weights(save_dir)
...
from scipy.ndimage import zoom
filename = '45-02s-13'
cwtms_export_merged_features = pd.read_pickle(r"C:\Users\Martin-PC\Biocomposite_analysis_isndcm\{filename}_128_512.pkl".format(filename=filename))
...
save_dir = save_dir_root + '\merged_features_encoder_clustering_chatGPT_32x64_6_clusters_v1_{filename}_32x64_energy_all'.format(filename=filename) +  ver_str + '.pkl'
...
# batch_size = 16
batch_size = 32
encoder_linear_regression_train = encoder_linear_regression.fit(x1_training_input_batch, 
