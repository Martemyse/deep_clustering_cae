...
all_features_to_keep = [3,8,17] #amp, risetime, FMXA
deep_features_to_keep = [18,23,24,25,26,27,28] # FMXA and deep
features_to_keep = all_features_to_keep
...
features_merged_labels_pure_selected_deep_FVTA = np.array(features_merged_labels_pure_selected_deep_FVTA_df)
# features_merged_labels_pure_selected_classical = np.array(features_merged_labels_pure_selected_classical)
...
# X = StandardScaler().fit_transform(x_pure_pred_enc)
# X = x_pure_pred_enc

# db = DBSCAN(eps=120.35, min_samples=50).fit(X) #very good but too aggressive
# db = DBSCAN(eps=0.7, min_samples=15).fit(X) #best od učer
db = DBSCAN(eps=7.9, min_samples=105).fit(X) # less aggressive than best, best za k-means

# db = DBSCAN(eps=5000.6, min_samples=10).fit(X) # less aggressive than best, CFE ch1
# db = DBSCAN(eps=20000.6, min_samples=45).fit(X) # less aggressive than best, GFE ch1

# db = DBSCAN(eps=0.66, min_samples=30).fit(X) # clustering GFE trying best
...
#%%
x_pure_pred_enc_df = pd.DataFrame(x_pure_pred_enc_unscaled_features)
x_pure_pred_enc_df['labels_DBscan'] =  labels_dbscan
# x_pure_pred_enc_df['labels_True_ctw'] =  labels_true


x_pure_pred_enc_df_dbscan_filtered = x_pure_pred_enc_df.drop(x_pure_pred_enc_df[x_pure_pred_enc_df['labels_DBscan'] == -1].index)

# hits_channel2_df.reset_index(inplace=True, drop=True)
x_pure_pred_enc_unscaled_features = features_merged_labels_pure_selected_deep_FVTA_df.drop(x_pure_pred_enc_df[x_pure_pred_enc_df['labels_DBscan'] == -1].index).reset_index(drop=True)
features_merged_dropped_dbscan = features_merged.drop(x_pure_pred_enc_df[x_pure_pred_enc_df['labels_DBscan'] == -1].index).reset_index(drop=True)
x_pure_pred_enc_unscaled_features = np.array(x_pure_pred_enc_unscaled_features)
...
for n_clusters in range_n_clusters:
    ...
    # sfs.fit(X)
    # sfs.get_support()*features_merged.columns
    # labels_true = clusterer.fit_predict(X)
    ...
    # dim_y = 2 # globoka 1 deep
    # dim_z = 0 # FCOG deep
    
    #classical features
    dim_x = 1 #energija CFE
    dim_y = 0# globoka 1 CFE
    dim_z = 2 # FCOG CFE
    ...
    ax.set_xlabel('Rise time', fontsize=20)
    ax.set_ylabel('Amplitude', fontsize=20)
    # ax.set_xlabel('Deep 1', fontsize=20)
    # ax.set_ylabel('Deep 2', fontsize=20)
    ax.set_zlabel('FMXA', fontsize=20, rotation = 0)
    #%
...
ax.scatter(hits_channel1_df['Time of Event'], np.log(Energy0), color=viridis_cols[0], label = "Cluster 1", marker = '.', alpha=0.5, linestyle = 'None', s=class_marker_sizes_amp0)
...
fig,ax = plt.subplots()
fig.set_size_inches(8, 5)
ax.plot(hits_channel1_df['Time of Event'], cumsum_labels_dict_true_3_label_0, color=viridis_cols[0], label = "Cluster 1")
...
ax.legend()
ax2=ax.twinx()
ax2.plot(hits_channel1_df['Time of Event'], hits_channel1_df[feature], color="orange", marker = '.', linestyle = 'None', markersize = 2.7)
ax2.set_ylabel(feature,color="orange",fontsize=14)
...
features_merged_copy = features_merged.iloc[:, all_features_to_keep]
...
mean_features = np.empty((n_clusters,features_merged_copy.shape[1]-1), dtype=float, order='C')
...
# print('FVTA_mean = '+ str(FVTA_mean))
features_merged_initial = features_merged.iloc[:, all_features_to_keep]
# breakpoint()
...
save_dir = save_dir_root + '\mean_features_df_{filename}'.format(filename=filename) +  ver_str + '.pkl'
pd.to_pickle(mean_features, save_dir)
save_dir = save_dir_root + '\mean_features_df_{filename}'.format(filename=filename) +  ver_str + '.xlsx'
